...
def parse_arguments():
    parser = ArgumentParser(description="Hide or reveal text in videos using steganography.",
                            epilog="Examples:\n"
                                   "  Hide a message: python vid.py hide --input input.mp4 --output output.mkv --message 'Privacy is a fundamental right' --codec FFV1 --public_key public.pem\n"
                                   "  Hide a message from a file: python vid.py hide --input input.mp4 --output output.mkv --message message.txt --codec FFV1 --public_key public.pem\n"
                                   "  Unhide a message: python vid.py unhide --input input.mkv --private_key private.pem --passphrase strong_password\n",
